...
def highlight_absolute_differences(string1, string2):
    ...
    res1 = []
    res2 = []
    for letter1, letter2 in zip_longest(string1, string2, fillvalue=""):
        if letter1 != letter2:
...
def date_formater(fields1_values, fields2_values):
    return highlight_absolute_differences(
        "/".join(str(int(x) if x else "").zfill(2) for x in fields1_values),
        "/".join(str(int(x) if x else "").zfill(2) for x in fields2_values),
    )


def name_formatter(fields1_values, fields2_values):
    field1_values = (str(x or "").title() for x in fields1_values)
    fields2_values = (str(x or "").title() for x in fields2_values)

    names1, names2 = zip_longest(
        *(
            highlight_relative_difference(val1, val2)
            for val1, val2 in zip(field1_values, fields2_values)
        )
    )
...
def gender_formatter(fields1_values, fields2_values):
    field1, = fields1_values
    field2, = fields2_values
    return highlight_absolute_differences(
        str(int(field1) if field1 else ""), str(int(field2) if field2 else "")
    )


def default_formatter(fields1_values, fields2_values):
    return highlight_absolute_differences(
        " ".join(str(x or "") for x in fields1_values),
        " ".join(str(x or "") for x in fields2_values),
    )
